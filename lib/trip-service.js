...
/**
 * Emergency vehicle reassignment for trips with issues
 * Validates: Requirements 13.2, 13.4
 * @param {string} tripId - Trip ID
 * @param {string} newVehicleId - New vehicle ID
 * @param {string} reason - Reason for reassignment
 * @param {string} userId - User performing the reassignment
 * @returns {Promise<Object>} Updated trip with audit log
 * @throws {Error} If validation fails or trip not found
 */
export async function emergencyVehicleReassignment(tripId, newVehicleId, reason, userId) {
  if (!reason || reason.trim().length === 0) {
    const error = new Error('Reason for reassignment is required')
    error.type = 'VALIDATION_ERROR'
    error.status = 400
    throw error
  }
  ...
  try {
    ...
    // Requirement 13.2, 13.4: Can only reassign vehicle if trip hasn't started (not IN_PROGRESS)
    if (currentTrip.status === 'IN_PROGRESS') {
      const error = new Error('Cannot reassign vehicle for a trip that is already in progress')
      error.type = 'BUSINESS_RULE_VIOLATION'
      error.status = 422
      throw error
    }
    ...
    const validation = await validateTripCreation({
    ...
    if (!validation.valid) {
      const error = new Error('Vehicle reassignment validation failed')
      error.type = 'BUSINESS_RULE_VIOLATION'
      error.status = 422
      error.errors = validation.errors
      throw error
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Update vehicle statuses if trip is dispatched
      ...
      // Requirement 13.5: Create audit log entry
      const auditLog = await tx.auditLog.create({
        data: {
          userId,
          action: 'EMERGENCY_VEHICLE_REASSIGNMENT',
          resource: 'trip',
          resourceId: tripId,
          metadata: {
            originalVehicleId,
...
/**
 * Emergency driver reassignment for trips
 * Validates: Requirements 13.3, 13.5
 * @param {string} tripId - Trip ID
 * @param {string} newDriverId - New driver ID
 * @param {string} reason - Reason for reassignment
 * @param {string} userId - User performing the reassignment
 * @returns {Promise<Object>} Updated trip with audit log and notifications
 * @throws {Error} If validation fails or trip not found
 */
export async function emergencyDriverReassignment(tripId, newDriverId, reason, userId) {
  if (!reason || reason.trim().length === 0) {
    const error = new Error('Reason for reassignment is required')
    error.type = 'VALIDATION_ERROR'
    error.status = 400
  ...
  try {
    ...
    if (!newDriver) {
      const error = new Error('The specified driver does not exist')
      error.type = 'VALIDATION_ERROR'
      error.status = 400
      throw error
    }
    ...
    if (!validation.valid) {
      const error = new Error('Driver reassignment validation failed')
      error.type = 'BUSINESS_RULE_VIOLATION'
      error.status = 422
      error.errors = validation.errors
      throw error
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const auditLog = await tx.auditLog.create({
        data: {
          userId,
          action: 'EMERGENCY_DRIVER_REASSIGNMENT',
          resource: 'trip',
          resourceId: tripId,
          metadata: {
            originalDriverId,
      ...
      // Requirement 13.3: Create notifications for both drivers
      const notifications = []

      // Notify original driver
      if (currentTrip.driver && currentTrip.driver.user) {
        const originalNotification = await tx.notification.create({
          data: {
            userId: currentTrip.driver.userId,
            type: 'TRIP_REASSIGNMENT',
            message: `Trip ${tripId} has been reassigned to another driver. Reason: ${reason}`,
            metadata: {
              tripId,
              reason,
              reassignmentType: 'emergency'
            }
          }
        })
        notifications.push(originalNotification)
      }
      ...
      if (newDriver && newDriver.user) {
        const newNotification = await tx.notification.create({
          data: {
            ...
            message: `You have been assigned to trip ${tripId} (emergency reassignment). Reason: ${reason}`,
            metadata: {
              tripId,
              reason,
              reassignmentType: 'emergency'
