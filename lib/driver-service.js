 ...
 * Historical trip records are preserved when driver is suspended
 * @param {string} driverId - Driver ID to suspend
 * @returns {Promise<Object>} Updated driver record
 */
...
/**
 * Get historical trip completion records for a driver (including SUSPENDED drivers)
 * Validates: Requirements 14.5
 * @param {string} driverId - Driver ID
 * @param {Object} options - Query options
 ...
 * @returns {Promise<Array>} List of historical trip completion records
 */
export async function getDriverHistoricalTrips(driverId, options = {}) {
  const where = {
    driverId
  }
...
/**
 * Get historical performance data for a driver (including SUSPENDED drivers)
 * Validates: Requirements 14.5
 * @param {string} driverId - Driver ID
 * @param {Object} dateRange - Date range for calculation
 * @param {Date} dateRange.start - Start date
 ...
 */
export async function getDriverHistoricalPerformance(driverId, dateRange) {
  // This function works the same as getDriverPerformance but explicitly
  // documents that it works for SUSPENDED drivers as well
  return getDriverPerformance(driverId, dateRange)
}
